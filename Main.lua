	...
	-- Widen the panel to fit 2 new buttons
		...
		EditorFrame.Input:GetPropertyChangedSignal("Text"):Connect(function()
			...
			highlightTask = task.delay(0.1, function()
				if highlighter and EditorFrame.Input then
...
-- // SECURITY & UI UPDATE //
if getgenv().PUNK_X_AUTH_TOKEN == "9a2f-punk-x-8812-secure-v2-441b" then
    
    -- 1. Security & Logic Setup
    getgenv().PUNK_X_AUTH_TOKEN = nil 
    KeyVailded = true -- This tells the internal logic "We are allowed to be here"
    
    -- 2. Start the Main UI Logic
    loadUI()

    -- 3. Update the Home Page Info (Green Box)
    if getgenv().PUNK_X_EXPIRY then
        task.spawn(function()
            ...
            -- Look for Main -> Pages -> Home -> Key (The Info Box, NOT the Overlay)
            repeat 
                task.wait(0.5)
